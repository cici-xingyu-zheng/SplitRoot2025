...
def get_stagewise_length(roots, choice, max_time, include_primary = False, corrected_timepoints = None):
    tot_rootlengths = []
    for root in roots:
...
def get_stagewise_num(roots, choice, max_time, include_primary = False, corrected_timepoints = None):
    tot_num_laterals = []
    for root in roots:
        print('Sample name:', root[0])
        print('------')
...
# def get_area_dfs(roots, labels, max_time, alphas, corrected_timepoints = None):
#     areas = []
#     indices = []
#     for root in roots:
...
#         lr_points = get_pointcloud(left_corrs,max_time)
#         rr_points = get_pointcloud(right_corrs,max_time)
...
#         timepoints = list(range(0, max_time, 8))

#     areas_df = pd.DataFrame(areas, columns = ['side', 'alpha', 'label'] + timepoints, index = indices)

#     label_dict = {'0': 'LL', '1': 'LH', '2': 'HL', '3': 'HH'}
#     conditions = [label_dict[label] for label in areas_df["label"].tolist()]
#     areas_df['condition'] = conditions
#     areas_df['condition-side'] = areas_df['condition'] + '-' + areas_df['side'] 

#     # aggregrate to four side-conditions:
#     # side_dict = {'LL-L':'homo-low', 'LL-R':'homo-low',
#     #             'LH-L':'hetero-low', 'LH-R':'hetero-high',
#     #             'HL-L':'hetero-high', 'HL-R':'hetero-low',
#     #             'HH-L':'homo-high', 'HH-R':'homo-high'}

#     # aggregrate to three side-conditions:
#     side_dict = {
#                 'LH-L':'hetero-low', 'LH-R':'hetero-high',
#                 'HL-L':'hetero-high', 'HL-R':'hetero-low',
#                 'HH-L':'homo-high', 'HH-R':'homo-high'}
...
#     areas_df['uniq-condition'] = [side_dict[l] for l in labelsides]
#     # condition_order = ['homo-high', 'hetero-high', 'hetero-low', 'homo-low']
#     condition_order = ['homo-high', 'hetero-high', 'hetero-low']
...
def get_area_dfs(roots, labels, max_time, alphas, corrected_timepoints=None):
    ...
    if corrected_timepoints is not None:
        ...
        # need to change to more flexible way
        timepoints = list(range(0, max_time, interval))
    ...
    # Define a comprehensive side_dict that handles all possible cases
    # full_side_dict = {
    #     'LL-L': 'homo-low', 'LL-R': 'homo-low',
    #     'LH-L': 'hetero-low', 'LH-R': 'hetero-high',
    #     'HL-L': 'hetero-high', 'HL-R': 'hetero-low',
    ...
    # condition_order = [cond for cond in ['homo-high', 'hetero-high', 'hetero-low', 'homo-low', 'homo-medium', 'hetero-medium', 'hetero-low(medium)'] 
    #                    if cond in available_conditions]
...
def get_primary_df(roots, snapshots, label_df):
    ...
    # Define a comprehensive side_dict for all possible cases
    # full_side_dict = {
    #     'LL-L': 'homo-low', 'LL-R': 'homo-low',
    #     'LH-L': 'hetero-low', 'LH-R': 'hetero-high',
    #     'HL-L': 'hetero-high', 'HL-R': 'hetero-low',
...
# def visualize_dataset(df, snapshot, measure, max = None, ylab = None, save_name = None):

#     order = ['homo-high', 'hetero-high', 'hetero-low', 'homo-low']

#     type2colors = {'len': [sns.color_palette("Set2")[-4],  '#d1e231', sns.color_palette("Set2")[-3], sns.color_palette("Set2")[-2]],
...
#     colors =type2colors[measure]
#     palette = dict(zip(order, colors))

#     fig, ax = plt.subplots(figsize = (6,6))
#     sns.barplot(data = df, x = 'uniq-condition', y = snapshot, palette = palette, order = order,
#                 capsize=.1)
#     sns.swarmplot(data = df, x = 'uniq-condition', y = snapshot, order = order,
...
#     if save_name:
#         fig.save(f'{save_name}.pdf', transparent = True)
...
# def perform_pairwise_ttests(df, group_column, value_column):
#     # Get unique groups
#     groups = df[group_column].cat.categories
...
#     for pair in pairs:
#         group1 = df[df[group_column] == pair[0]][value_column]
#         group2 = df[df[group_column] == pair[1]][value_column]
...
#     rejected, adjusted_p_values, _, _ = multipletests(p_values, method='fdr_bh') # bonferroni
    
#     # Add adjusted p-values to results
#     for (pair, result), adj_p in zip(results.items(), adjusted_p_values):
...
#     groups = [group for _, group in df.groupby(group_column)[value_column]]
